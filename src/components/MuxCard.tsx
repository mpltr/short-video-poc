...
export default function MuxCard({ video, onClick }: Props) {
  ...
  return (
    <div
      onClick={onClick}
      className="relative aspect-[9/14] w-full overflow-hidden rounded-md bg-black text-white shadow-md cursor-pointer"
    >
      {video.thumbnail ? (
        <img
          src={video.thumbnail}
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 to-black/20 p-2 text-xs">
        ...
        <div className="font-medium text-sm leading-tight line-clamp-2">
          {video.title || "Three Thursday tips at Thurles – including a..."}
        </div>
      </div>
    </div>
  );
